...
function refreshSocketAuth(socket) {
  ...
  return socket.userDbId;
}

function bindPlayerAccount(game, socketId, userDbId) {
  if (!userDbId) return;
  const player = game.players.find((p) => p.id === socketId);
  if (player) player.userDbId = userDbId;
}

function refreshLobbyPlayerAccounts(lobby) {
  if (!lobby?.game || !lobby.playerIds) return;
  for (const socketId of lobby.playerIds) {
    const s = io.sockets.sockets.get(socketId);
    if (!s) continue;
    const userDbId = refreshSocketAuth(s);
    bindPlayerAccount(lobby.game, socketId, userDbId);
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('syncAuth', () => {
    const userDbId = refreshSocketAuth(socket);
    const code = socketToLobby.get(socket.id);
    if (code) {
      const lobby = lobbies.get(code);
      if (lobby?.game) bindPlayerAccount(lobby.game, socket.id, userDbId);
    }
    socket.emit('authSynced', { userDbId: userDbId || null });
  });
  ...
  socket.on('createLobby', (playerName) => {
    ...
    game.addPlayer(socket.id, name, userDbId || null);
  ...
  socket.on('joinLobby', (data) => {
    ...
    const userDbId = refreshSocketAuth(socket);
    const success = lobby.game.addPlayer(socket.id, name, userDbId || null);
  ...
  socket.on('nextQuestion', () => {
    ...
    if (game.nextQuestion()) {
      ...
      try {
        refreshLobbyPlayerAccounts(lobby);
